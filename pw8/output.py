...
def pickleWrite(studentList:list) -> None:
    with open(getPath() + 'students.dat', 'wb') as file:
        for st in studentList:
            dump(st, file)

class PickleThread(Thread):
    path:str
    stList:list
    def __init__(self, path:str,studentList:list):
        Thread.__init__(self) 
        self.path = path + 'studentPickle.dat'
        self.stList = studentList
...
def pickleWriteBackground(studentList:list) -> None:
    pickleThread = PickleThread(getPath(),studentList)
    pickleThread.start()
...
class cmdFunction:
    ...
    @staticmethod
    def pickleWriteBackgroundCmd() -> None:
        pickleWriteBackground(studentList)
#endregion
